...
from function.config import feature_dataset_path as fdp
from function.config import dataset_path as dp
...
def swtFeatureMap(normalized_img, wavelet='db3', level=7):
    # 提取规范化后（矩形区域）的图片特征
    swt_list = np.zeros((normalized_img.shape[0], level + 7, normalized_img.shape[1]), dtype=np.uint8)  # (40, 8, 512)
    for index, value in enumerate(normalized_img):
        # cA7, cD7, cD6, cD5, cD4, cD3, cD2, cD1
        swt_coeffs = pywt.swt(data=value, wavelet=wavelet, level=level, )
        #trim_approx=True
        for coeff in swt_coeffs:
            for i in coeff:
                i[i > 0] = 1
                i[i < 0] = 0
        print(swt_coeffs)  #(7,2,512)
        swt_list[index] = np.array(swt_coeffs).reshape(-1,512)
    swt_list = swt_list.swapaxes(0, 1)  # (8, 40, 512)
    feature = swt_list.reshape((-1, 512))  # (320, 512)   转换成512列
